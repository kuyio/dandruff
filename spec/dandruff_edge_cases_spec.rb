  ...
  describe 'Edge Cases and Malformed HTML' do
    ...
    describe 'large input handling' do
      it 'handles very large HTML strings' do
        ...
        clean = dandruff.sanitize(large_content)
    ...
    describe 'mutation XSS resilience' do
      it 'performs at least two passes by default' do
        count = 0
        dandruff.add_hook(:before_sanitize_elements) { |_node, _data, _config| count += 1 }
        dirty = '<div><script>alert(1)</script><style>body{color:black}</style></div>'
        dandruff.sanitize(dirty)
        expect(count).to be >= 2
        dandruff.remove_all_hooks
      end
      ...
      it 'stabilizes mutated SVG payloads across passes' do
        ...
        clean = dandruff.sanitize(dirty, sanitize_until_stable: true, mutation_max_passes: 3)
        second_pass = dandruff.sanitize(clean, sanitize_until_stable: true, mutation_max_passes: 3)
      ...
      it 'allows disabling stabilization with pass_limit 0' do
        ...
        dandruff.sanitize(dirty, sanitize_until_stable: true, pass_limit: 0)
